...
import decision_attacker_binary_and_rel_scores
import random
import string
import time
import sys
...
for num_secrets in secrets_to_try:
    ...
    for trial in range(0, 200, num_secrets):
        ...
        dbreacher = dbreacher_impl_binary_search.DBREACHerImpl(control, table, num_secrets, maxRowSize, fillerCharSet, ord('*'))

        attacker = decision_attacker_binary_and_rel_scores.decisionAttacker(dbreacher, guesses)
        while not success:
...
endAttack = time.time()
print("Total time running in seconds: " + str(endAttack - startAttack))