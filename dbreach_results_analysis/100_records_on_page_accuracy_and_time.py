...
for c in ["snappy"]:
    ...
    with open(text_type) as csvfile:
        ...
        for row in reader:
            ...
            #     if "Total DB Queries This Round:" not in row[0] and 'Total time spent this round in seconds:' not in row[0] and 'ref_score_db_queries' not in row[0] and 'true_label' not in row[0] and 'PERCENT' not in row[0] and 'QUERIES' not in row[0] and 'setup_db_queries' not in row[0]:
            #         print(row)
            #         # setup_queries[records_on_page].append(float(row[0]))
            #         # ref_scores_db_queries[records_on_page].append(float(row[1]))
            #         # guess_db_queries[records_on_page].append(float(row[2]))
            #         total_db_queries[records_on_page].append(float(row[1]))
